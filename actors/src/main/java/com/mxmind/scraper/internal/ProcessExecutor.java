 ...
 * @since 1.0-SNAPSHOT
 */
public final class ProcessExecutor implements Executor {
    ...
    public void exec(String path) {
        ...
        try (final IndexWriter writer = openWriter(indexDir)) {
            ...
            // each task creates new nested threads accordingly to file parts
            // amount;
            final long endScraping = stopWatch.getElapsedTime();
            out.format("Scraper found %d videos with quality: %s, by time %.1fs \n\n", result.totalHits,
                    Main.props.getString("scraper.video.quality", "p144"), Math.floor(endScraping / second));
